// MainChat.tsx
// Sanctuary Main Chat with agentic overlays and unified MultiModalChatBar
  ...
  return (
    <SanctuaryLayout>
      <Box sx={{ pt: 8, pb: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '90vh' }}>
        <Typography level="h3" sx={{ mb: 2, fontWeight: 800 }}>Main Chat</Typography>
        <Box sx={{ width: 680, minHeight: 320, background: 'rgba(236,72,153,0.07)', borderRadius: 3, mb: 4, p: 2 }}>
          {messages.length === 0 ? (
            <Typography sx={{ color: '#888', fontSize: 16 }}>Start a conversation with Mama Bear…</Typography>
          ) : (
            messages.map((msg, i) => (
        ...
        <FloatingMamaBear mood="friendly" />
      </Box>
    </SanctuaryLayout>
  );
};
